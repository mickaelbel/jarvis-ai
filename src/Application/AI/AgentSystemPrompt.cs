...
public static class AgentSystemPrompt
{
    ...
    public static string Build(IReadOnlyList<AIToolDefinition> tools, string executionMode = "Speed")
    {
        ...
        if (executionMode == "Show")
        {
            ...
            sb.AppendLine("  - Toujours utiliser computer_action action=start app=\"nom\"");
            sb.AppendLine("  - Le tool fait automatiquement : touche Win → tape le nom → Entrée");
            sb.AppendLine("  - JAMAIS utiliser un autre outil pour ouvrir une app.");
            sb.AppendLine("  - UNE SEULE action computer_action suffit.");
            sb.AppendLine();
            sb.AppendLine("NOMS RECONNUS PAR WINDOWS (dans la barre de recherche) :");
            sb.AppendLine("  calculator = Calculatrice | notepad = Bloc-notes | paint = Paint");
            sb.AppendLine("  edge = Edge | chrome = Chrome | firefox = Firefox");
            sb.AppendLine("  word = Word | excel = Excel | explorer = Explorateur");
            sb.AppendLine("  code = VS Code | regedit = Registre | cmd = Terminal");
            sb.AppendLine("  task manager = Gestionnaire des tâches | control panel = Panneau de config");
            sb.AppendLine("  Tu peux aussi taper le nom français (calculatrice, bloc-notes, peinture)");
            sb.AppendLine("  car la recherche Windows les reconnaît. Mais JAMAIS de noms inventés.");
            sb.AppendLine();
