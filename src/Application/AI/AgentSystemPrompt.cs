...
public static class AgentSystemPrompt
{
    public static string Build(IReadOnlyList<AIToolDefinition> tools)
    {
        ...
        sb.AppendLine("- FORMATAGE (IMPORTANT) : utilise du vrai markdown. NE COMMENCE JAMAIS une ligne par « > ». N'écris JAMAIS toutes les lignes d'un même paragraphe avec « > » devant. NE GÉNÈRE JAMAIS de balises HTML (<h2>, <a>, <span>, <div>, id=, class=, etc.) — utilise UNIQUEMENT la syntaxe markdown : ## pour les titres, **gras**, *italique*, |tableaux|, - listes.");
        sb.AppendLine("   EXEMPLE de bon format :");
        sb.AppendLine("   ## Titre de section");
        sb.AppendLine();
        sb.AppendLine("   | Modèle | Pourquoi | Année |");
        sb.AppendLine("   |--------|----------|-------|");
        sb.AppendLine("   | Ferrari 250 GTO | Design iconique | 1962 |");
        sb.AppendLine("   | Porsche 917 | Innovation technique | 1968 |");
        sb.AppendLine();
        sb.AppendLine("   Chaque élément de tableau sur sa propre ligne, et des retours à la ligne entre les sections.");
        sb.AppendLine("   EXEMPLE de mauvais format (À ÉVITER) : « >Modèle >Pourquoi >Année >Ferrari... » ou « <h2 id=...>Titre ».");
