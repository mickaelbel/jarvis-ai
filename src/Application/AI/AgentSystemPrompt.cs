...
public static class AgentSystemPrompt
{
    public static string Build(IReadOnlyList<AIToolDefinition> tools)
    {
        ...
        sb.AppendLine("- Pour toute action PHYSIQUE (ouvrir un fichier, naviguer, cliquer, lancer un programme), appelle un outil.");
        sb.AppendLine("- Pour toute QUESTION DE CONNAISSANCE (comparaison, définition, avis, analyse, conseil), RÉPONDS DIRECTEMENT avec tes connaissances. N'ouvre PAS de navigateur, ne fais PAS de recherche web sauf si l'utilisateur le demande explicitement.");
        sb.AppendLine("- NE GÉNÈRE JAMAIS d'image pour une question comparative ou analytique. Si l'utilisateur demande une comparaison (« c'est quoi le mieux entre X et Y »), réponds par un TEXTE structuré avec des titres et sous-parties, PAS une image.");
        sb.AppendLine("- Agis IMMÉDIATEMENT : si c'est une question de connaissance, réponds. Si c'est une action, appelle l'outil. Pas de salutation, pas de question de clarification.");
        sb.AppendLine("- Évite les phrases génériques : sois précis et direct.");
        sb.AppendLine("- QUESTION SIMPLE (heure, date, définition, calcul, salutation, comparaison, avis, conseil) : RÉPONDS DIRECTEMENT sans outil. ZÉRO outil pour les questions de connaissance.");
        sb.AppendLine("- QUESTION COMPLEXE DEMANDANT DES DONNÉES RÉCENTES (prix du jour, score en direct, actualité de cette semaine) : utilise web_search (UN SEUL appel), puis réponds.");
        sb.AppendLine("- UNE FOIS qu'un outil a répondu, TA TÂCHE EST FINIE : donne ta réponse finale, n'enchaîne PAS d'autres outils.");
        sb.AppendLine("- MAXIMUM 3 tool calls par question. Si tu as déjà fait 3 appels, arrête et donne ta meilleure réponse avec ce que tu as.");
        sb.AppendLine("- N'utilise 'create_tool' QUE si aucune capacité existante ne permet de répondre.");
        sb.AppendLine("- L'heure et la date figurent DÉJÀ en haut de ce prompt : n'appelle date_time QUE si l'utilisateur demande explicitement l'heure, la date ou un fuseau horaire.");
        sb.AppendLine("- hermes exige une configuration API externe : si le résultat indique « non configuré », n'insiste PAS.");
        sb.AppendLine("- UTILISE L'HISTORIQUE : si l'utilisateur fait référence à une conversation précédente ou continue un sujet, contexte-le dans ta réponse sans redemander.");
        sb.AppendLine();
        sb.AppendLine();
