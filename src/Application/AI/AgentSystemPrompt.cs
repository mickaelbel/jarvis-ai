...
public static class AgentSystemPrompt
{
    ...
    public static string Build(IReadOnlyList<AIToolDefinition> tools, string executionMode = "Speed")
    {
        ...
        if (executionMode == "Show")
        {
            ...
            sb.AppendLine("    Win+R → tape le NOM OFFICIEL → Entrée.");
            ...
            sb.AppendLine();
            sb.AppendLine("NOMS OFFICIELS WINDOWS (à utiliser dans Win+R) :");
            sb.AppendLine("  calc = Calculatrice | notepad = Bloc-notes | mspaint = Paint");
            sb.AppendLine("  msedge = Edge | chrome = Chrome | firefox = Firefox");
            sb.AppendLine("  winword = Word | excel = Excel | explorer = Explorateur");
            sb.AppendLine("  code = VS Code | regedit = Registre | cmd = Terminal");
            sb.AppendLine("  taskmgr = Gestionnaire des tâches | control = Panneau de config");
            sb.AppendLine("  JAMAIS taper les noms français (calculateur, navigeteur, etc.)");
            sb.AppendLine();
        ...
        sb.AppendLine();
            ...
            sb.AppendLine("  « Lance la calculatrice » = computer_action action=start app=\"calc\"");
            sb.AppendLine("  « Ouvre Paint » = computer_action action=start app=\"mspaint\"");
            ...
            sb.AppendLine("  JAMAIS taper de nom français dans Win+R — utiliser les noms officiels.");
